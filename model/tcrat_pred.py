...
class TcratPred(pl.LightningModule):
    def __init__(self, args):
        super(TcratPred, self).__init__()
        self.args = args

        self.save_hyperparameters()

        self.encoder_tcn = EncoderTCN(self.args)
        ...
        pre_trained_checkpoint_path = "checkpoints/filename.ckpt"  
        self.load_pretrained_model(pre_trained_checkpoint_path)

    def load_pretrained_model(self, checkpoint_path):
        # Load the pre-trained model checkpoint
        pretrained_model_state_dict = torch.load(checkpoint_path)
        # Update your model's parameters with the pre-trained model's parameters
        self.load_state_dict(pretrained_model_state_dict, strict=False)
    ...
    def forward(self, batch):
        ...
        out_encoder_tcn = self.encoder_tcn(displ_cat, agents_per_sample)
        out_agent_gnn = self.agent_gnn(
            out_encoder_tcn, centers_cat, agents_per_sample)
    ...
    def calc_prediction_metrics(self, preds, gts):
        ...
        error_per_t = np.linalg.norm(
            preds - np.expand_dims(gts, axis=1), axis=-1)
...
class Chomp1d(nn.Module):
    ...
    def forward(self, x):
        # Remove extra padded elements at the end to maintain causal structure
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        ...
        self.chomp1 = Chomp1d(padding) # Ensure no leakage from future timestamps
        self.relu1 = ReLU()
        ...
        # Stack as a sequential residual block
    ...
    def init_weights(self):
        # Initialize weights with small variance for stability
...
class EncoderTCN(nn.Module):
    def __init__(self, args):
        ...
        self.input_size = 3  # Expected number of input channels/features
        self.hidden_size = args.latent_size
        self.num_layers = 1

        # TCN backbone with constant channel size per layer
        self.tcn = TemporalConvNet(
            num_inputs=self.input_size,
            num_channels=[self.hidden_size]*self.num_layers
        )

    def forward(self, x, agents_per_sample):
        # TCN expects inputs of shape (batch_size, num_channels, sequence_length)
        x = x.transpose(1, 2)
        out = self.tcn(x)

        # Extract last time-step representation (acts as temporal summary) 
        out = out[:, :, -1]
